...
function SideNav({ isMobile = false, onClose }) {
  ...
  useEffect(() => {
    ...
    const myMenuList = routeIds
      .map((x) => NAVIGATION_ROUTES.find((y) => y.id == x))
      .filter((x) => x);
  ...
  return (
    <div
      className={`border shadow-md h-screen p-3 md:p-5 ${isMobile ? 'w-full' : ''} overflow-y-auto`}
    >
      ...
      <div className="flex flex-col gap-1 md:gap-0">
        {menuList.map((menu, index) => (
          <Link key={index} href={menu.path} onClick={handleLinkClick}>
            <h2
              className={`flex items-center gap-2 md:gap-3 text-sm md:text-md p-3 md:p-4
          text-slate-500
          hover:bg-primary
          hover:text-white
          cursor-pointer
      ...
      <div
        className={`${isMobile ? 'relative mt-5' : 'absolute bottom-5 left-3 md:left-6'} flex justify-center items-center gap-2`}
      >
        <Profile />
        <DropdownMenu>
