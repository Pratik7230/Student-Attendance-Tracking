...
const GetAllGradesForSubject = (selectedSubjectId) => axios.get(`/api/grade?selectedSubjectId=${selectedSubjectId}`);
...
const GetAttendanceList = (gradeId, month, selectedSubjectId) =>
    axios.get(`/api/attendance?gradeId=${gradeId}&month=${month}&selectedSubjectId=${selectedSubjectId}`);

const GetStudentAttendanceList = (studentUserId, month, selectedSubjectId) =>
    axios.get(`/api/attendance?studentUserId=${studentUserId}&month=${month}&selectedSubjectId=${selectedSubjectId}`);

const GenerateReport = (gradeId, from, to, selectedSubjectId) =>
    axios.get(`/api/generate-report?gradeId=${gradeId}&from=${from}&to=${to}&selectedSubjectId=${selectedSubjectId}`, { responseType: "blob" });


const MarkAttendance = (data) => axios.post('/api/attendance', data);
const MarkAbsent = (studentId, day, date, subjectId, gradeId) =>
    axios.delete(`/api/attendance?studentId=${studentId}&day=${day}&date=${date}&subjectId=${subjectId}&gradeId=${gradeId}`);

const TotalPresentCountByDay = (date, gradeId,selectedSubjectId) =>
    axios.get(`/api/dashboard?date=${date}&gradeId=${gradeId}&selectedSubjectId=${selectedSubjectId}`);

// 🔐 Authentication & Password Reset
const Login = (user) => axios.post('/api/login', user);
const GenerateOTP = ({ email }) => axios.post('/api/reset-password-generate-otp', { email });
...
const GetTeacherSubjects = (userId,teacher_email) => axios.get(`/api/teacher_subjects?userId=${userId}&teacher_email=${teacher_email}`);
const UpdateTeacherSubjects = (teacherUserId,selectedSubjects) => axios.put(`/api/teacher_subjects?teacherUserId=${teacherUserId}`,selectedSubjects);
const GetStudentSubjects = (userId) => axios.get(`/api/student_subjects?studentUserId=${userId}`);
