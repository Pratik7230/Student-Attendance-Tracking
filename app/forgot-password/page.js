...
export default function ForgotPassword() {
  ...
  const handleEmailSubmit = async (data) => {
    setEmail(data.email);
    setLoading(true)
    GlobalApi.GenerateOTP({ email: data.email }).then(() => {
      setStep(2);
    }).catch(() => {
  ...
  const handleOtpSubmit = async (data) => {
    setOtp(data.otp);
    setLoading(true)
    GlobalApi.ValidateOTP({ email, otp: data.otp }).then(() => {
      setStep(3);
    }).catch(() => {
  ...
  const handlePasswordSubmit = async (data) => {
    ...
    setLoading(true)
    GlobalApi.ResetPasswordWithOTP({ email, otp, password: data.password }).then(() => {
