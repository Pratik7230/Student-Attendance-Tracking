...
export async function POST(req) {
    try {
        ...
        if (Array.isArray(data)) {
            ...
            const user_result = await db.insert(USER).values(data.map(x => (
                {
                    name: x.name,
                    email: x.email,
                    role_id: 3,
            ...
            return NextResponse.json({ message: "Bulk students added successfully!", result: student_result }, { status: 201 });
        } else {
            ...
            return NextResponse.json({ message: "Student added successfully!", result }, { status: 201 });
        }
    } catch (error) {
...
export async function DELETE(req) {
    ...
    const user_result = await db.delete(USER)
